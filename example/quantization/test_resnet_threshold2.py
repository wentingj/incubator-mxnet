...
while True:
    r = input('Enter ratio to saturate threshold: ')
    if r < 0:
        break
    new_quantile_dict = {}
    for k, v in quantile_dict.items():
        if k.find('conv') != -1:
            new_quantile_dict[k] = (v[0] * float(r), v[1] * float(r))
        else:
            new_quantile_dict[k] = v
    ...
    cqsym = mx.quantization.calibrate_quantized_sym(qsym, new_quantile_dict, calib_table_type)
