  ...
  if (!ndim) {
    ...
    MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, DType, {
      ...
      BROADCAST_NDIM_SWITCH(ndim, NDim, {
        ...
        size_t workspace_size_l = ReduceWorkspaceSize<NDim, DType>(s, lhs.shape_, req[0], out.shape_);
        size_t workspace_size_r = ReduceWorkspaceSize<NDim, DType>(s, rhs.shape_, req[1], out.shape_);
...
inline void BinaryBroadcastBackwardUseInImpl(const OpContext& ctx,
  ...
  size_t workspace_size_l = ReduceWorkspaceSize<ndim, DType>(s, lgrad.shape_, req[0], ograd.shape_, lhs.shape_, rhs.shape_);
  size_t workspace_size_r = ReduceWorkspaceSize<ndim, DType>(s, rgrad.shape_, req[1], ograd.shape_, lhs.shape_, rhs.shape_);
  size_t workspace_size = std::max(workspace_size_l, workspace_size_r);
  Tensor<xpu, 1, char> workspace =
    ctx.requested[0].get_space_typed<xpu, 1, char>(Shape1(workspace_size), s);
