...
#include "../nn/../elemwise_op_common.h"
#include "../nn/./mkldnn/mkldnn_ops-inl.h"
#include "../nn/./mkldnn/mkldnn_base-inl.h"
#if MXNET_USE_NNPACK == 1
#include "../nn/./nnpack/nnpack_convolution-inl.h"
#endif  // MXNET_USE_NNPACK

namespace mxnet {
namespace op {
DMLC_REGISTER_PARAMETER(Quantized_conv2dParam);

static inline index_t AddPad(index_t dsize, index_t pad) {
  return dsize + 2 * pad;
}

static inline std::vector<std::string> ListArguments(const Quantized_conv2dParam& param_) {
  if (!param_.no_bias) {
...
static void ConvolutionCompute_CPU(const nnvm::NodeAttrs& attrs,
  ...
  if (SupportMKLDNNConv(inputs[0])) {
    MKLDNNQuantized_conv2dForward(attrs, ctx, inputs, req, outputs);
    return;
  }
#endif
}
...
static bool ConvolutionShape(const nnvm::NodeAttrs& attrs,
  ...
  const Quantized_conv2dParam& param_ = nnvm::get<Quantized_conv2dParam>(attrs.parsed);
  if (!param_.no_bias) {
...
inline static bool ConvStorageType(const nnvm::NodeAttrs& attrs,
  ...
  const Quantized_conv2dParam& param = nnvm::get<Quantized_conv2dParam>(attrs.parsed);
...
inline static bool BackwardConvStorageType(const nnvm::NodeAttrs& attrs,
  ...
  if (dev_mask == mshadow::cpu::kDevMask) {
    *dispatch_mode = DispatchMode::kFComputeEx;
    (*out_attrs)[qconv::kData] = kMKLDNNStorage;
    ...
    (*out_attrs)[qconv::kWeight] = kDefaultStorage;
    if (!param.no_bias)
      (*out_attrs)[qconv::kBias] = kDefaultStorage;
...
static void Quantized_conv2dParamParser(nnvm::NodeAttrs* attrs) {
  using namespace mshadow;
  Quantized_conv2dParam param_;
  try {
...
struct ConvolutionGrad {
  ...
  std::vector<nnvm::NodeEntry> operator()(const nnvm::NodePtr& n,
    ...
    const Quantized_conv2dParam& param = nnvm::get<Quantized_conv2dParam>(n->attrs.parsed);
    std::vector<nnvm::NodeEntry> heads(ograds.begin(), ograds.end());
    heads.push_back(n->inputs[qconv::kData]);
    heads.push_back(n->inputs[qconv::kWeight]);
    if (!param.no_bias)
      heads.push_back(n->inputs[qconv::kBias]);
...
.set_num_inputs([](const NodeAttrs& attrs) {
  const Quantized_conv2dParam& params = nnvm::get<Quantized_conv2dParam>(attrs.parsed);
  return params.no_bias ? 2 : 3;
})
.set_num_outputs(1)
.set_attr_parser(Quantized_conv2dParamParser)
...
.add_arguments(Quantized_conv2dParam::__FIELDS__());
